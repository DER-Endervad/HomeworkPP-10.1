...
public:
    ...
    std::unique_ptr<tridiagonal_matrix> clone() {
        auto output = std::make_unique<tridiagonal_matrix>(*this);
        return output;
    }

    std::unique_ptr<tridiagonal_matrix> clone2() {
        auto output = std::make_unique<tridiagonal_matrix>(this->m_down, this->m_upper, this->m_middle);
        return output;
    }
};
...
int main()
{
    ...
    auto matrix_clone2 = matrix->clone2();
